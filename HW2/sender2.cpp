...
int main(int argc, char ** argv)
{
    ...
    reliable_receive_packet_alarm(socket_fd, argv[3], seq_num, strlen(argv[3]), 1, 0);
    ...
    {
        reliable_receive_packet_alarm(socket_fd, local_file_buffer, seq_num, num_read, 0, 0);
        seq_num++;
    }

    //struct reliable_packet last_packet(htonl(0), htonl(1), htonl(0), htonl(0));
    
    reliable_receive_packet_alarm(socket_fd, NULL, seq_num, 0, 0, 1);
